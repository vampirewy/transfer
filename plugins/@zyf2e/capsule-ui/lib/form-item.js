...
/* harmony default export */ var form_itemvue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    sizeClass: function sizeClass() {
      return this.elFormItemSize || (this.$CAPSULE || {}).size;
    }
  },
  data: function data() {
    return {
  ...
  methods: {
    ...
    onFieldChange: function onFieldChange() {
       ...
       * 当form下任意表单改变就会触发
       */
      this.elForm && this.elForm.$emit('change', this.prop);
  ...
  mounted: function mounted() {
    /**
     * element-ui在实现table的fixed样式时，是通过渲染两个table达成的，一个table用于正常的展示，一个table用于fixed展示
     * 但是，如果table中含有form-item，并配置了校验规则，则校验规则会被add两次,导致上层回调执行两次
     * 通过把tableBody注入进form-item，判断是否配置了fixed解决，如果fixed了，则不进行第二次注册
     */
    // 如果tableBody不存在，说明没嵌入进表格中，可以添加规则，如果存在,那么fixed属性一定**不能**存在
    var allowAddValid = !this.tableBody || !this.tableBody.fixed;
    if (this.prop && allowAddValid) {
