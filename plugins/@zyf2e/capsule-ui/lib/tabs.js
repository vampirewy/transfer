...
/* harmony default export */ var tabsvue_type_script_lang_js_ = ({
  ...
  watch: {
    ...
    // 获取当前tab的index值
    panes: function panes(newVal) {
      var _this2 = this;

      this.$nextTick(function () {
        _this2.currentIndex = _this2.panes.filter(function (v) {
          if (v.paneName === _this2.currentName) {
            // 如果有name
            return v;
          }
        })[0].index;
  ...
  methods: {
    calcPaneInstances: function calcPaneInstances() {
      ...
      if (this.$slots.default) {
        ...
        var panesChanged = !(panes.length === this.panes.length && panes.every(function (pane, index) {
          return pane === _this3.panes[index];
        }));
    ...
    handleTabClick: function handleTabClick(tab, tabName, event) {
      ...
      this.$emit('tab-click', tab, event); // 雨女无瓜
    },
      ...
      if (this.currentName !== value && this.beforeLeave) {
        ...
        if (before && before.then) {
          before.then(function () {
            changeCurrentName();
            _this4.$refs.nav && _this4.$refs.nav.removeFocus();
          }, function () {
  ...
  render: function render(h) {
    ...
    var panels = h(
      'div',
      { 'class': 'el-tabs__content', attrs: { dir: this.dir }
      },
      [this.$slots.default]
    );
