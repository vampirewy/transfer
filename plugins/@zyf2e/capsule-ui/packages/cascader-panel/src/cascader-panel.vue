...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'capsule-ui/src/utils/util';
  ...
  methods: {
    ...
    syncActivePath() {
      ...
      if (!isEmpty(activePath)) {
        const nodes = activePath.map(node => this.getNodeByValue(node.getValue())).filter(Boolean);
