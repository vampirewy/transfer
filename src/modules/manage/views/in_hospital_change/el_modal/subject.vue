<template>
  ...
  <div class="user-follow">
    <el-table :data="table.list" style="width: 100%" align="center" ref="table" class="openTable">
      ...
      <el-table-column prop="departmentName" label="科室名称" min-width="60px" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column prop="arriveRate" label="到诊率" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{ scope.row.arriveRate | getResult }}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await this.$api.InhospitalChange.getDepartmentCheckTopList(reqBody);
      /* const res = {
        data: {
          data: {
            data: [
    ...
    async exportList() {
      ...
      const res = await this.$api.InhospitalChange.exportDepartmentCheckTopList(reqBody);
      const { data } = res.data;
