  ...
  >
    <div class="body">
      ...
      >
        ...
        <el-table-column prop="name" label="运动名称" show-overflow-tooltip width="130px">
        </el-table-column>
        <el-table-column prop="sportType" label="运动类型">
          <template slot-scope="scope">
            <span v-if="scope.row.sportType === 1">日常运动</span>
            <span v-if="scope.row.sportType === 2">健身类</span>
            <span v-if="scope.row.sportType === 3">耐力类</span>
            <span v-if="scope.row.sportType === 4">跳绳类</span>
            <span v-if="scope.row.sportType === 5">舞蹈类</span>
            <span v-if="scope.row.sportType === 6">游泳类</span>
          </template>
        </el-table-column>
        <el-table-column prop="sportSort" label="运动分类">
          <template slot-scope="scope">
            <span v-if="scope.row.sportSort === 1">有氧</span>
            <span v-if="scope.row.sportSort === 2">力量</span>
            <span v-if="scope.row.sportSort === 3">柔韧性</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="met" label="代谢当量"></el-table-column>
        <el-table-column prop="" label="运动禁忌疾病"  show-overflow-tooltip width="130px">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    onVisible(value) {
      if (!value) {
        this.$jDynamic.hide({ component: 'elImportSport' });
      }
    },
    /**
     * 保存
