    ...
    <div class="wltable">
      ...
      <div class="table-content">
        <ul v-for="item in datalist" :key="item.id">
          <li>{{item.userName}}</li>
          <li v-for="it in item.interveneCountDTOList" :key="it.id">
            {{reverseMessage(it.interveneCount)}}</li>
        </ul>
        <ul class="total">
          <li>合计</li>
        ...
        <ul class="percentage">
          <li>百分比</li>
          <li v-for="ites in pagenumber" :key="ites.id">{{reverseMessage(ites)}}</li>
          <li>-</li>
          <li>-</li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    getarchives() {
      ...
      this.$api.statics.intervene(sendDataGet).then((res) => {
        this.datalist = res.data.data;
        this.formdata = this.datalist[0].interveneCountDTOList;
        let Countes = 0;
        const fourCount = [];
        for (let i = 0; i < this.datalist[0].interveneCountDTOList.length; i++) {
          fourCount.push(0);
        }
        this.datalist.forEach((value) => {
          // const count = [];
          value.interveneCountDTOList.forEach((valList, valIndex) => {
            if (valIndex <= value.interveneCountDTOList.length - 2) {
              Countes += valList.interveneCount;
              fourCount[valIndex] += valList.interveneCount;
        ...
        fourCount.forEach((val, index) => {
          if (index < fourCount.length - 2) {
            const numbere = (val / Countes) * 100;
            if (Countes === 0) {
              this.pagenumber.push('0.00');
            } else {
