  ...
  <div class="dashboard-editor-container">
    ...
    <div class="wltable">
      <div class="tabletop">
        <ul>
          <li>建档人</li>
          <li v-for="u in formdata" :key="u.id">{{u.clientGridName}}</li>
        </ul>
      </div>
      <div class="table-content">
        <ul v-for="item in datalist" :key="item.id">
          <li>{{item.userName}}</li>
          <li v-for="it in item.chartDTOList" :key="it.id">{{it.clientCount}}</li>
        </ul>
        <ul class="total">
          <li>合计</li>
...
export default {
  ...
  methods: {
    ...
    getarchives() {
      ...
      this.$api.statics.administer(sendDataGet).then((res) => {
        this.datalist = res.data.data;
        this.formdata = this.datalist[0].chartDTOList;
        let Countes = 0;
        const fourCount = [];
        for (let i = 0; i < this.datalist[0].chartDTOList.length; i++) {
          fourCount.push(0);
        }
        this.datalist.forEach((value) => {
          value.chartDTOList.forEach((valList, valIndex) => {
            if (valIndex <= value.chartDTOList.length - 2) {
