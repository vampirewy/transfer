<template>
  <div class="create-edit">
    ...
    >
      ...
      <div class="tableTopDoDiv" style="margin-top: 10px;margin-bottom: 20px"
           v-if="routeType === 1 && reservationFieldList.length > 0">
        <div class="divRightTitleDiv">
      ...
      <el-row>
        <el-col :span="item.type === 1 || item.type === 2 ? 6 : 24"
                v-for="item in reservationFieldList" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    async getServiceItemDetail(id) {
      ...
      const { data } = res.data;
      this.reservationFieldList = data.reservationFieldList;
      console.log(data);
    },
    /**
     * 获取服务详情
    ...
    async getDetail() {
      ...
      this.getServiceItemDetail(this.form.serviceItemId).then(() => {
        if (data.reservationInfo && data.reservationInfo !== '{}') {
          ...
          console.log(this.reservationFieldList);
          this.reservationFieldList.forEach((val) => {
            const Val = val;
    ...
    async save() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          let reqBody = '';
          const Obj = {};
          this.reservationFieldList.forEach((value) => {
